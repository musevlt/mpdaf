...
def test_levels(signal, levels):
    ...
    h_coefficients_list = numpy.array([1 / 16.0, 1 / 4.0, 3 / 8.0, 1 / 4.0,
                                       1 / 16.0])  # See book by Starck
    h_length = len(h_coefficients_list)
    signalSize = signal.size
    if levels > numpy.log2((signalSize - 1.0) / (h_length - 1.0)):
...
def wavelet_transform(signal, levels):
    ...
    for i, element in enumerate(signal):
        if numpy.isnan(element):
            # If we have missing values, set the signal to 0
            signal[i] = 0.0
    signalSize = signal.size
