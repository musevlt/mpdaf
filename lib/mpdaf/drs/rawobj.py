...
class Channel(object):
    ...
    mask (boolean mask) : Arrays that contents TRUE for overscanned pixels, FALSE for the others
    """
...
class RawFile(object):
        ...
        if self.progress:
            ...
            while (True):
                ...
                if completed == num_tasks: 
                    ...
                    break
                output = "\r (%i%% done)"%(float(completed)/float(num_tasks)*100.0)
                sys.stdout.write("\r\x1b[K"+output.__str__())
                sys.stdout.flush()
        ...
        for ifu,data in processresult:
            ...
            for k in range(1,NB_SUBSLICERS+1):
                ...
                for l in range(1,NB_SLICES+1):
                    ...
                    wr_row = NB_SLICES - slit_position[l - 1] + 12*(24-ifu)
                    wr_col = k * NB_SPEC_PER_SLICE 
                    #test[wr_row,wr_col-NB_SPEC_PER_SLICE:wr_col] = data[noslice-1,::-1]
                    white_ima[wr_row,wr_col-NB_SPEC_PER_SLICE:wr_col] = data[noslice-1,:]
    ...
    def _plot_ifu_slice_on_white_image(self,ifu,slice):
        ...
        plt.plot(np.arange(wr_col,wr_col+76),np.ones(76)*(wr_row+1),'r-')
        plt.plot(np.ones(2)*wr_col,np.arange(wr_row,wr_row+2),'r-')
        plt.plot(np.ones(2)*(wr_col+75),np.arange(wr_row,wr_row+2),'r-') 
...
#        for s in range(1,49):      
#            xstart = mask_chan.header['HIERARCH ESO DET SLICE%d XSTART'%s].value - OVERSCAN
#            xend = mask_chan.header['HIERARCH ESO DET SLICE%d XEND'%s].value - OVERSCAN
#            if xstart>(mask_chan.header["ESO DET CHIP NX"].value/2.0):
#                xstart -= 2*OVERSCAN
#            if xend>(mask_chan.header["ESO DET CHIP NX"].value/2.0):
#                xend -= 2*OVERSCAN
#            ystart = mask_chan.header['HIERARCH ESO DET SLICE%d YSTART'%s].value - OVERSCAN
#            yend = mask_chan.header['HIERARCH ESO DET SLICE%d YEND'%s].value - OVERSCAN
#            if ystart>(mask_chan.header["ESO DET CHIP NY"].value/2.0):
#                ystart -= 2*OVERSCAN
#            if yend>(mask_chan.header["ESO DET CHIP NY"].value/2.0):
#                yend -= 2*OVERSCAN
#                
#            plt.plot(np.arange(xstart,xend+1),np.ones(xend+1-xstart)*ystart,'g-')
#            plt.plot(np.arange(xstart,xend+1),np.ones(xend+1-xstart)*yend,'g-')
#            plt.plot(np.ones(yend+1-ystart)*xstart,np.arange(ystart,yend+1),'g-')
#            plt.plot(np.ones(yend+1-ystart)*xend,np.arange(ystart,yend+1),'g-')
#            plt.annotate('%02d'%s, xy=(xstart, yend+1),  xycoords='data',textcoords='data', color='g')  
        
        self.x1 = mask_chan.header['HIERARCH ESO DET SLICE1 XSTART'].value - OVERSCAN
        self.x2 = mask_chan.header['HIERARCH ESO DET SLICE48 XEND'].value - 2*OVERSCAN
        
        xstart = mask_chan.header['HIERARCH ESO DET SLICE%d XSTART'%slice].value - OVERSCAN
        xend = mask_chan.header['HIERARCH ESO DET SLICE%d XEND'%slice].value - OVERSCAN
        if xstart>(mask_chan.header["ESO DET CHIP NX"].value/2.0):
            xstart -= 2*OVERSCAN
        if xend>(mask_chan.header["ESO DET CHIP NX"].value/2.0):
            xend -= 2*OVERSCAN
        ystart = mask_chan.header['HIERARCH ESO DET SLICE%d YSTART'%slice].value - OVERSCAN
        yend = mask_chan.header['HIERARCH ESO DET SLICE%d YEND'%slice].value - OVERSCAN
        if ystart>(mask_chan.header["ESO DET CHIP NY"].value/2.0):
            ystart -= 2*OVERSCAN
        if yend>(mask_chan.header["ESO DET CHIP NY"].value/2.0):
            yend -= 2*OVERSCAN
        
        plt.plot(np.arange(xstart,xend+1),np.ones(xend+1-xstart)*ystart,'r-')
        plt.plot(np.arange(xstart,xend+1),np.ones(xend+1-xstart)*yend,'r-')
        plt.plot(np.ones(yend+1-ystart)*xstart,np.arange(ystart,yend+1),'r-')
        plt.plot(np.ones(yend+1-ystart)*xend,np.arange(ystart,yend+1),'r-')
        plt.annotate('%02d'%slice, xy=(xstart, yend+1),  xycoords='data',textcoords='data', color='r')  
        if same_raw is False:
            self.rawima = self.get_channel(chan).get_trimmed_image()
        #raw_ima.plot(colorbar='h',cmap=cm.copper)
            ...
            if event.inaxes is not None:
                if (event.x<self.fig.canvas.get_width_height()[0]/2):
                    ...
                    if ifu == self.plotted_chan:
                        self._plot_slice_on_raw_image(ifu,slice,same_raw=True)
                    else:
                    ...
                    ax = plt.subplot(1,2,2)
                    ax.clear()
                    self._plot_slice_on_raw_image(self.plotted_chan,slice,same_raw=True)
                    ax = plt.subplot(1,2,1)
                    ax.clear()
                    self._plot_ifu_slice_on_white_image(self.plotted_chan,slice)
    ...
    def plot_white_image(self,mask=None):
        ...
        if mask is None:
            import mpdaf
            self.mask_file = mpdaf.__path__[0]+'/drs/mumdatMask_1x1/PAE_July2013.fits'
        #create image
        ...
        print 'To select on other channel/slice, click on the images with the right mouse button.'
...
def _process_white_image(arglist):
    ...
    for slice in range(1,49):
        ...
        for i in range(NB_SPEC_PER_SLICE):
            spe_slice_75pix[i] = integrate.quad(f,x[i],x[i+1],full_output=1)[0] / new_step
