...
class DisplayPixTable(object):
    ...
    def __init__(self, pixtable, cube):
        """creates a DisplayPixTable object and verifies that pixtable and cube are compatible"""
    ...
    def _det_display(self, date, pix, ima, spe, ifu_limits, l, exp, sky, lbda, sky_scale, sky_cmap, det_scale, det_cmap, det_vmin, det_vmax):
        """display in detector mode for one exposure
        
        :param exp: exposure number.
        :type sky: integer or None
        
        :param sky: (y, x, size, shape) extract an aperture on the sky, defined by a center in degrees (y, x), a shape ('C' for circular, 'S' for square) and size in arcsec (radius or half side length).
        :type sky: (float, float, float, char)

        :param lbda: (min, max) wavelength range in Angstrom.
        :type lbda: (float, float)
        
        :param sky_scale: The stretch function to use for the scaling of the sky image (default is 'linear').
        ...
        :param det_scale: The stretch function to use for the scaling of the detector images (default is 'linear').
        ...
        :param det_vmin: Minimum pixel value to use for the scaling of the detector images.
        If None, det_vmin is set with the IRAF zscale algorithm.
        :type det_vmin: float
        
        :param det_vmax: Maximum pixel value to use for the scaling of the detector images.
        If None, det_vmax is set with the IRAF zscale algorithm.
        :type det_vmax: float
        """
        
        fig = plt.figure()
        plt.figtext(0.1, 0.05, 'Pixtable %s %s'%(self.pixtable, date), fontsize=10)
        ...
        for ifu in list_ifu:
            ...
            detima = subsubpix.reconstruct_det_image(ystart=ystart,ystop=ystop)
            del subsubpix
        ...
        for ifu,detima in zip(list_ifu,list_detima):
            ...
            im = detima.plot(title='%02d'%ifu,scale=det_scale,vmin=det_vmin,vmax=det_vmax,extent=[limits[0][1],limits[1][1],limits[0][0],limits[1][0]],cmap=det_cmap)
            if iplot !=0 :
                plt.ylabel('')
                plt.yticks([])
            else:
        ...
        for ifu in list_ifu:#plot ifus limits on image
            ...
            plt.annotate('%02d'%ifu, xy=(0, (ymin+ymax)/2.0),  xycoords='data',textcoords='data', color='b')
        im = ima.plot(colorbar='h',scale=sky_scale,cmap=sky_cmap)
        im.get_axes().set_axis_off()
        print 'plot corresponding spectrum ...'
        plt.subplot2grid((5,nplots),(0,colspan_ima),rowspan=2,colspan=int(nplots/2))
        spe.plot()    
        plt.xlim(lbda[0],lbda[1])

    def det_display(self, sky, lbda, sky_scale='linear', sky_cmap=cm.copper, det_scale='linear', det_cmap=cm.copper, det_vmin=None, det_vmax=None):
        ...
        :param det_vmin: Minimum pixel value to use for the scaling of the detector images. If None, det_vmin is set with the IRAF zscale algorithm.
        :type det_vmin: float
        
        :param det_vmax: Maximum pixel value to use for the scaling of the detector images. If None, det_vmax is set with the IRAF zscale algorithm.
        :type det_vmax: float
        """
        pix = PixTable(self.pixtable)
        cub = CubeDisk(self.cube)
        ...
        #ifu grid
        ifu_low = pix.get_keywords('HIERARCH ESO DRS MUSE PIXTABLE LIMITS IFU LOW')
        ifu_high = pix.get_keywords('HIERARCH ESO DRS MUSE PIXTABLE LIMITS IFU HIGH')
        nifu = pix.nifu
        ...
        ifu_limits = dict((_ifu,(_ymin,_ymax))  for _ifu,_ymin,_ymax in zip(np.arange(ifu_high,ifu_low-1,-1), ymin, ymax))
        ...
        if subcub.shape[0] == 0 or subcub.shape[1] == 0 or subcub.shape[2] == 0:
        ...
        try:
            ...
            for exp in exposures:
                ...
                self._det_display(date, pix, ima, spe, ifu_limits, l, exp, sky, lbda, sky_scale, sky_cmap, det_scale, det_cmap, det_vmin, det_vmax) 
        except:
            self._det_display(date, pix, ima, spe, ifu_limits, l, None, sky, lbda, sky_scale, sky_cmap, det_scale, det_cmap, det_vmin, det_vmax)
        
        

    def _slice_display(self, date, pix, ima, spe, slice_limits, ima_center_p, ima_center_q, dp_slice, dq_slice, exp, sky, lbda, sky_scale, sky_cmap, slice_scale, slice_cmap, slice_vmin, slice_vmax):
        ...
        :param slice_scale: The stretch function to use for the scaling of the slice images (default is 'linear').
        ...
        :param slice_vmin: Minimum pixel value to use for the scaling of the slice images.
        If None, det_vmin is set with the IRAF zscale algorithm.
        :type slice_vmin: float
        
        :param slice_vmax: Maximum pixel value to use for the scaling of the slice images.
        If None, det_vmax is set with the IRAF zscale algorithm.
        ...
        for ifu,slice,p in zip(distance[:,1],distance[:,2],distance[:,3]):            
            if q<0:
                q_label=0
            elif q>=ima.shape[1]:
                ...
                q_label=q
            plt.annotate('%02d/%02d'%(ifu,slice), xy=(q_label,p-0.2),  xycoords='data',textcoords='data', color='b')
            pmin = p - dp_slice/2.0
            pmax = p + dp_slice/2.0
            qmin = q - dq_slice/2.0
            qmax = q + dq_slice/2.0
            plt.plot(np.arange(qmin,qmax+1,1),np.ones(qmax-qmin+2)*pmin,'b-')
            plt.plot(np.arange(qmin,qmax+1,1),np.ones(qmax-qmin+2)*pmax,'b-')
            plt.plot(np.ones(pmax-pmin+2)*qmin,np.arange(pmin,pmax+1,1),'b-')
            plt.plot(np.ones(pmax-pmin+2)*qmax,np.arange(pmin,pmax+1,1),'b-')
        im = ima.plot(colorbar='h',scale=sky_scale,cmap=sky_cmap)
        im.get_axes().set_axis_off()
        
        #plot corresponding spectrum
        print 'plot corresponding spectrum ...'
        plt.subplot2grid((9,nplots),(0,colspan_ima),rowspan=3,colspan=nplots-colspan_ima)
        spe.plot()    
        plt.xlim(lbda[0],lbda[1])
        ...
        iplot = 0
        for ifu,slice,sliceima in zip(distance[:,1],distance[:,2],list_sliceima):
            limits = sliceima.wcs.get_range()
            plt.subplot2grid((9,nplots),(4,iplot),rowspan=3)
            im = sliceima.plot(title='%02d/%02d'%(ifu,slice),scale=slice_scale,vmin=slice_vmin,vmax=slice_vmax,extent=[limits[0][1],limits[1][1],limits[0][0],limits[1][0]],cmap=slice_cmap)
            if iplot !=0 :
                plt.ylabel('')
                plt.yticks([])
            else:
    ...
    def slice_display(self, sky, lbda, sky_scale='linear', sky_cmap=cm.copper, slice_scale='linear', slice_cmap=cm.copper, slice_vmin=None, slice_vmax=None):
        """display in slice mode
        ...
        :param slice_vmin: Minimum pixel value to use for the scaling of the slice images. If None, det_vmin is set with the IRAF zscale algorithm.
        :type slice_vmin: float
        
        :param slice_vmax: Maximum pixel value to use for the scaling of the slice images. If None, det_vmax is set with the IRAF zscale algorithm.
        :type slice_vmax: float
        """
        cub = CubeDisk(self.cube)
        pix = PixTable(self.pixtable)
        ...
        for i in range(nifu):
            list_p[i,0:12] = i*dp_ifu + np.arange(12)*dp_slice + dp_slice/2.0 -0.5
            list_p[i,12:24] = i*dp_ifu + np.arange(12)*dp_slice + dp_slice/2.0 -0.5
            list_p[i,24:36] = i*dp_ifu + np.arange(12)*dp_slice + dp_slice/2.0 -0.5
            list_p[i,36:48] = i*dp_ifu + np.arange(12)*dp_slice + dp_slice/2.0 -0.5
        ...
        slice_limits = dict(((ifu,slice),(p,q)) for ifu,slice,p,q in zip(list_ifu,list_slice,list_p,list_q))
        ...
        try:
            ...
            exposures = np.unique(col_exp)
            for exp in exposures:
               self._slice_display(date,pix, ima, spe, slice_limits, ima_center_p, ima_center_q, dp_slice, dq_slice, exp, sky, lbda, sky_scale, sky_cmap, slice_scale, slice_cmap, slice_vmin, slice_vmax)
        except:
            self._slice_display(date,pix, ima, spe, slice_limits, ima_center_p, ima_center_q, dp_slice, dq_slice, None, sky, lbda, sky_scale, sky_cmap, slice_scale, slice_cmap, slice_vmin, slice_vmax)
