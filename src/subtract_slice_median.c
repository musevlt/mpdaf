...
// typ=0 -> mpdaf_divide_slice_median
// typ=1 -> mpdaf_subtract_slice_median
int initPix(Pix * array, int npix, int* ifu, int* sli, double* data, double* lbda, int* mask, double* skyref_flux, double* skyref_lbda, int skyref_n, int typ) {
  int i, count=0;
  if (typ==0)
  {
      for (i = 0; i < npix; i++)
...
// typ=0 -> mpdaf_divide_slice_median
// typ=1 -> mpdaf_subtract_slice_median
void mpdaf_slice_median(double* result, double* result_stat ,double* corr, int* npts, int* ifu, int* sli, double* data,  double* lbda, int npix, int* mask, double* skyref_flux, double* skyref_lbda, int skyref_n, int typ)
{
    Pix* pixels = (Pix*) malloc(npix * sizeof(Pix));
    int npix2;
    npix2 = initPix(pixels, npix, ifu, sli, data, lbda, mask, skyref_flux, skyref_lbda, skyref_n, typ);
    sortPix(pixels, npix2);
    ...
    for(i=0;i<npix2;i++)
      {
    	if ((pixels[i].sli == sl) && (pixels[i].ifu==chan))
    	  imax = imax +1;
    	else
    	  {
    	    if(imin!=imax)
    	      {
		index = 48*(chan-1)+sl-1;
    		int num=imax-imin+1;
    		if (num%2 == 0)
    		  corr[index] = (pixels[imin+(int)(num/2)].data + pixels[imin+(int)(num/2)-1].data)/2;
    		else
    ...
    {
        #pragma omp parallel shared(result, data, corr, ifu, sli, npix) private(index)
        {
            #pragma omp for
	    for(n=0; n<npix; n++)
	    {
